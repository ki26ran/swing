...
def run_bt(overrides):
    s = get_strategy('keltner_rsi')
    for k,v in overrides.items():
        setattr(s, k, v)
    try:
        trades, _ = run_backtest(s, months=months,
                                 universe=get_strategy_universe('keltner_rsi'),
...
print("=== KELTNER RSI OPTIMIZATION ===\n", flush=True)
base_r = run_bt({})
...
print("\n--- Sweep: keltner_period ---", flush=True)
for v in [15,20,25]:
    r=run_bt({'keltner_period':v})
...
print("\n--- Sweep: keltner_multiplier ---", flush=True)
for v in [1.5,2.0,2.5,3.0]:
    r=run_bt({'keltner_multiplier':v})
...
print("\n--- Sweep: rsi_period ---", flush=True)
for v in [10,14,20]:
    r=run_bt({'rsi_period':v})
    if r: print(f"  rsi_per={v:>2}: P&L={r['total_pnl']:>8,.0f} WR={r['winrate']:>5}% PF={r['profit_factor']} Trades={r['total_trades']:>2} Score={r['score']}", flush=True)

print("\n--- Sweep: rsi_long_min ---", flush=True)
for v in [45,50,55,60]:
    r=run_bt({'rsi_long_min':v})
    if r: print(f"  rsi_long={v:>2}: P&L={r['total_pnl']:>8,.0f} WR={r['winrate']:>5}% PF={r['profit_factor']} Trades={r['total_trades']:>2} Score={r['score']}", flush=True)

print("\n--- Sweep: rsi_short_max ---", flush=True)
for v in [35,40,45,50]:
    r=run_bt({'rsi_short_max':v})
    if r: print(f"  rsi_short={v:>2}: P&L={r['total_pnl']:>8,.0f} WR={r['winrate']:>5}% PF={r['profit_factor']} Trades={r['total_trades']:>2} Score={r['score']}", flush=True)

print("\n--- Sweep: volume_multiplier ---", flush=True)
for v in [1.5,2.0,3.0,4.0]:
    r=run_bt({'volume_multiplier':v})
    if r: print(f"  vol_mult={v:.1f}: P&L={r['total_pnl']:>8,.0f} WR={r['winrate']:>5}% PF={r['profit_factor']} Trades={r['total_trades']:>2} Score={r['score']}", flush=True)
...
combos = [
    ...
    {'rsi_long_min':50,'rsi_short_max':45},
    {'rsi_long_min':55,'rsi_short_max':45},
    {'keltner_period':25,'keltner_multiplier':1.5},
    {'rsi_long_min':50,'volume_multiplier':2.0},
    {'adx_threshold':15,'keltner_multiplier':2.5},
    {'keltner_period':25,'rsi_long_min':55,'rsi_short_max':45},
]
for p in combos:
    r=run_bt(p)
